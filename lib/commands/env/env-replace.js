          ...
          keys.forEach(key => {
            ...
            if (value) {
              fileContent = isFisProject
                ? fileContent.replace(
                    new RegExp(RegExpUtils.escape(`\${${key}}`), 'g'),
                    `\${${value}}`
                  )
                : fileContent
                    .replace(
                      new RegExp(RegExpUtils.escape(`process.env.${key}`), 'g'),
                      `process.env['${value}']`
                    )
                    .replace(
                      new RegExp(RegExpUtils.escape(`env.raw.${key}`), 'g'),
                      `env.raw['${value}']`
                    );
            }
          });
          ...
          htmlKeys.forEach(htmlKey => {
            ...
            if (value) {
              fileContent = fileContent.replace(
                new RegExp(RegExpUtils.escape(`%${htmlKey}%`), 'g'),
                `%${value}%`
              );
            }
          });
