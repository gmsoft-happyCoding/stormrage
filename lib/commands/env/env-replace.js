...
async function replaceEnv({ localDir, opts }) {
  ...
  const reactRegexpStr = `process\\.env\\.(${mapKeys.join('|')})`;
    ...
    allFile.forEach(filePath => {
      ...
      if (matched || htmlMatched) {
        if (matched) {
          ...
          // Key值的匹配模式
          const keyPattern = isFisProject ? /\$\{(.*)\}/ : /process\.env\.(\w+)/;
          // 提取出结果中的key
          const keys = uniqMatch.map(i => i.match(keyPattern)[1]);
          // 遍历keys，并从map中取得对应的值，然后在content中进行替换
          keys.forEach(key => {
            const value = replaceConf.envMap[key];
            if (value) {
              fileContent = isFisProject
                ? fileContent.replaceAll(`\${${key}}`, `\${${value}}`)
                : fileContent.replaceAll(`process.env.${key}`, `process.env['${value}']`);
              console.log('isFisProject: ', isFisProject);
            }
          });
        }
        ...
        if (htmlMatched) {
          ...
          htmlKeys.forEach(htmlKey => {
            ...
            if (value) {
              fileContent = fileContent.replaceAll(`%${htmlKey}%`, `%${value}%`);
            }
          });
        }
