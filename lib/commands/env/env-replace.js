/**
 * 1. 在项目根目录下执行
 * 2. 脚本将帮助您快速替换项目内的环境变量引用，例如：${interface_xcj} => ${business.interface_xcj}
 * 3. 如果想要使用脚本进行环境变量快速替换，请将项目的环境变量均定义在business段下，包括网关，domain等等的
 * 4. 执行 stormrage env --replace
 *
 * 风险：
 * 该脚本的替换规则是基于配置的动态正则表达式，并非基于AST，因此可能会存在误替换的风险
 * 使用前请检查您的项目内是否存在如下的情景，如果存在，请不要使用此脚本进行批量替换
 *
 * 危险场景示例：
 *
 * 项目yml配置： business.open_api : 'https://openapi.zcygov.cn'
 * 项目内源码：
 * const open_api = 'xxxx'
 * const url = `${open_api}/xxx`
 *
 * 阐释：
 * 上述场景中，在使用此脚本进行替换时，会将${open_api}替换为${business.open_api}，导致url变量的值变为
 * const url = `${business.open_api}/xxx`
 * 从而导致语法错误，主要原因是基于正则的匹配无法鉴别命中的字符串到底是环境变量引用，还是ES6字符串模板语法
 */

const fs = require('fs');
const path = require('path');
const inquirer = require('inquirer');
const { ConfHelper } = require('../../utils/ConfHelper');
const { getTargetEnv } = require('./env');

// 无需处理的文件夹以及文件
const ignoreDir = ['node_modules', 'modules'];
// 需要处理的源码文件后缀
const targetFileSuffix = [
  'js',
  'jsx',
  'ts',
  'tsx',
  'json',
  'html',
  'css',
  'less',
  'scss',
  'sass',
  'tpl',
  'es',
  'html',
];

// 是否是文件
function isFile(filePath) {
  return fs.statSync(filePath).isFile();
}

// 获取目标目录及子目录下所有文件的绝对路径
function getAllFile(dirPath, fileExt) {
  // 如果传入了额外的文件后缀规则，则合并
  let exts = targetFileSuffix;
  if (fileExt) {
    exts = [...targetFileSuffix, ...fileExt];
  }

  const currentPath = process.cwd();
  // 获取当前目录下的所有文件和文件夹，并过滤掉黑名单中的项目
  const allItems = fs.readdirSync(dirPath).filter(i => !ignoreDir.includes(i));
  /** 通过验证的文件列表 */
  const validFiles = [];
  /** 通过验证的文件夹列表 */
  const validDirs = [];

  allItems.forEach(item => {
    const itemPath = path.join(currentPath, item);
    // 如果是文件，且后缀名在目标后缀名列表中，则加入到有效文件列表中
    if (isFile(itemPath) && exts.includes(item.split('.').pop())) {
      validFiles.push(itemPath);
    }
    // 如果是文件夹，则加入到有效文件夹列表中
    if (!isFile(itemPath)) {
      validDirs.push(itemPath);
    }
  });

  // 递归下级目录
  validDirs.forEach(subDir => {
    process.chdir(subDir);
    validFiles.push(...getAllFile(subDir, exts));
    process.chdir(path.join(subDir, '..'));
  });

  return validFiles;
}

function readFileContent(filePath) {
  return fs.readFileSync(filePath, 'utf-8');
}
function writeFileContent(filePath, fileContent) {
  fs.chmodSync(filePath, 0o777);
  return fs.writeFileSync(filePath, fileContent, 'utf-8');
}

async function replaceEnv({ localDir, opts }) {
  let rootDir = localDir || process.cwd();
  // 获取到的环境变量配置信息
  const allEnv = await getTargetEnv(rootDir, opts);

  // 枚举出业务环境变量，并删除business前缀
  const keys = Object.keys(allEnv.envConf)
    .filter(i => i.startsWith(ConfHelper.CONF_DEFAULT_FIELD_NAME))
    .map(i => i.replace(`${ConfHelper.CONF_DEFAULT_FIELD_NAME}.`, ''));

  // 要替换的目标串模式，基于配置的动态正则表达式
  const regexpStr = `\\$\\{(?!(business|gateway|hosts|pdf-preview).)(${keys.join('|')})\\}`;
  const replacePattern = new RegExp(regexpStr, 'g');

  const allFile = getAllFile(rootDir, opts.fileExt ? opts.fileExt.split(',') : undefined);

  // 询问是否开始替换
  const result = await inquirer.prompt([
    {
      type: 'list',
      name: 'goon',
      message: '是否开始替换？（请确认本地没有未提交的修改，避免代码出现循坏后无法回退）',
      choices: ['yes', 'no'],
    },
  ]);

  if (result.goon === 'yes') {
    allFile.forEach(filePath => {
      const fileContent = readFileContent(filePath);
      const match = fileContent.match(replacePattern);
      if (match) {
        // 命中规则，进行替换并写出
        const replacedContent = fileContent.replace(replacePattern, '${business.$2}');
        writeFileContent(filePath, replacedContent);
      }
    });
    console.log('[Done] 环境变量替换完成，请检查项目是否正常运行');
  } else {
    console.log('[Done] 放弃替换');
  }
}

module.exports = { replaceEnv };
