...
async function make({ localDir, opts }) {
  await NpmHelper.getLatestVersion('app-online-assessment');
  return;
  try {
    ...
    const currentMakeVersion = NpmHelper.getLatestVersion();
    fs.writeFileSync(
      filePath,
      JSON.stringify({
        ...lodash.pick(projectInfo, ['name']),
        version: ReleaseHelper.getMakeNextVersion(currentMakeVersion),
        // 环境变量占位映射表
        envMap: fullEnvConf,
        // Make执行时间
        makeTime: timeStr,
    ...
    if (opts.dest) {
      ...
      await zip(buildDir, typeof opts.dest === 'string' ? opts.dest : 'D:\\发布结果');
    } else {
      // 将产出的目标产物上传到成品库
      ...
      // 将身份口令写到环境变量
      process.env.NPM_AUTH_TOKEN = 'NpmToken.78c3b93d-1084-3c4f-862b-56574af554c8';
      // 执行发布
      exec('npm publish');
    }
