...
async function make({ localDir, opts }) {
  ...
  if (isProject(rootDir)) {
    ...
    console.log('当前路径：%s', process.cwd());
  ...
  if (isMonoProject(rootDir) && !package) {
    ...
    // 判断当前环境是不是Linux
    if (isLinux) {
      const svnOptIndex = options.findIndex(i => i.includes('--svn'));
      if (svnOptIndex >= 0) {
        options.splice(svnOptIndex, 1);
      }
      // 移除reinstall参数
      const reinstallOptIndex = options.findIndex(i => i.includes('--reinstall'));
      if (reinstallOptIndex >= 0) {
        options.splice(svnOptIndex, 1);
      }
    } else {
      // 非Linux环境
      const svnOptIndex = options.indexOf('--svn');
      if (svnOptIndex >= 0) {
        options.splice(svnOptIndex, 2);
      }
      // 移除reinstall参数
      const reinstallOptIndex = options.indexOf('--reinstall');
      if (reinstallOptIndex >= 0) {
        options.splice(svnOptIndex, 1);
      }
    }
  ...
  await createZOperator(baseConf, rootDir, metaInfo);
