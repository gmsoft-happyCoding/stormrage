...
async function createZOperator(baseConf, rootDir, originalEnvConf) {
  const projectName = originalEnvConf['business.project-name'];
  // ================================================= 处理CI相关文件 =================================================
  ...
  try {
    ...
    if (runmoduleIsExist) {
      ...
      if (!runmodelsFileContent.includes(projectName)) {
        /**
        ...
        fileLine.push(projectName);
        runmodelsFileContent = fileLine.join('\n');
      }
    } else {
      // 不存在文件，直接写出项目名即可
      runmodelsFileContent = projectName;
    }
