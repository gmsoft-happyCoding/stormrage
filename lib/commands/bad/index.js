...
async function bad({ projectDir, package, destDir, program }) {
  // svn参数优先
  let dir = program.svn ? await fetchProject(program.svn) : projectDir;

  // 询问必要信息
  const answers = await asking(dir, package);
