...
function buildFileTree(allTreeNode, nextFileTask) {
  ...
  allTreeNode.forEach(outCursor => {
    nextFileTask.forEach(innerCursor => {
      ...
      // 内层游标文件名是否符合Hash规则，如果不符合，说明不是目标文件
      const isValidFile = FileHelper.isHashFileNamePattern.test(innerCursor.fileItem.baseName);
      if (isValidFile) {
        const innerFileHash = innerCursor.fileItem.baseName.match(
          FileHelper.isHashFileNamePattern
        )[0];
        // 外层文件内容包含内层文件名中的Hash的，计入依赖关系
        const hasDependencyRelation = outCursor.fileItem.fileContent.includes(innerFileHash);
  ...
  taskFileNode.forEach(task => {
    ...
    for (const dependencyItem of dependencyIterator) {
      ...
      if (task.fileItem.isDirty) {
        // 待替换的文件名，不包含后缀
        const replaceFileNames = task.fileItem.hashBaseNameHistory
          .slice(0, task.fileItem.hashBaseNameHistory.length - 1)
          .map(i => i.replace(/\.[\da-z]+$/, ''));

        const replacePattern = new RegExp(`(${replaceFileNames.join('|')})`, 'ig');

        // 更新文件内容中的Hash文件名，set钩子将自动更新Hash文件名
        dependencyItemFile.fileContent = dependencyItemFile.fileContent.replace(
          replacePattern,
          task.fileItem.hashBaseName.replace(/\.[\da-z]+$/, '')
        );
...
async function recalculateHashUseTree(allFiles, modifyFiles, ignoreDir) {
  ...
  const ignoreDirCorrect = ignoreDir
    ? `(?<=${isWindows ? path.sep + path.sep : path.sep})(${ignoreDir
        ...
        .replace(/\//g, isWindows ? path.sep + path.sep : path.sep)
        .replace(/,/g, '|')})`
    : null;
