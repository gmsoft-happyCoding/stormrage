...
async function deploy({ projectName, env, room, opts }) {
  console.log('[1/5] 创建暂存目录...');
  ...
  console.log('[2/5] 拉取成品库文件...');
  // 安装成品文件，如果指定的了版本则使用指定的版本
  const installShell = opts.target
    ? `npm --registry ${NpmHelper.REGISTRY_PRODUCT} install ${projectName}@${target}`
  ...
  console.log('[3/5] 获取目标环境配置信息...');
  // 获取目标环境配置信息
  const { data: confStr } = await axios.get(confUrl);
