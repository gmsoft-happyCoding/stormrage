...
function replaceSourceMap(targetFileContent, mappingLines, envConf, confKeys) {
  const minKeyLength = Math.min(...confKeys.map(i => getKeyPlaceHolders(i).length));
  const matchKeyPattern = new RegExp(
    getKeyPlaceHolders(`(${confKeys.map(RegExpUtils.escape).join('|')})`)
  );
  let resultContent = targetFileContent;
  let hasChange = false;

  const contentLine = targetFileContent.split('\n');
  contentLine.forEach((line, index) => {
    let tempLine = line;
    const lineMapping = mappingLines[index];
    if (lodash.isNil(lineMapping)) {
      return;
    }
    if (!lodash.every(lineMapping, lineMapping.length === 1 && lodash.isEmpty(lineMapping[0]))) {
      let targetBaseIndex = 0;
      for (let i = 0; i < lineMapping.length; i++) {
        const currentBase = targetBaseIndex + lineMapping[i][0];
        const [start, end] = [
          currentBase,
          lineMapping[i + 1] !== undefined ? currentBase + lineMapping[i + 1][0] : undefined,
        ];

        targetBaseIndex += lineMapping[i][0];
        // 目标代码片段
        const targetCodeFragment = tempLine.slice(start, end);

        if (targetCodeFragment.length < minKeyLength) {
          continue;
        }

        const match = targetCodeFragment.match(matchKeyPattern);
        ...
        const [before, middle, after] = [
          tempLine.substr(0, start),
          tempLine.substr(start, targetCodeFragment.length),
          tempLine.substr(start + targetCodeFragment.length),
        ];

        // 替换中间段，循环替换，直到没有匹配项；坑麻了，单个VLQ中居然可能包含数个环境变量占位符
        const newMiddle = replaceAllEnv(middle, matchKeyPattern, envConf); // middle.replace(match[0], envConf[match[1]]);
        tempLine = before + newMiddle + after;

        // 计算VLQ差值，修正VLQ游标
        const vlqEffect = newMiddle.length - middle.length;
        if (lineMapping[i + 1] !== undefined) {
          lineMapping[i + 1][0] += vlqEffect;
        }
      }
    }
    contentLine[index] = tempLine;
  });

  resultContent = contentLine.join('\n');

  return { resultContent, resultMapping: mappingLines, hasChange };
}
...
function replaceEnvForSourceMap(envConf, confKeys, fileItem, fileContent) {
  ...
  const mappingLines = sourceMapInfo.mappings
    .split(';')
    .map(i => i.split(',').map(i => Base64VLQUtils.decodeMappings(i)));
  ...
  sourceMapInfo.mappings = resultMapping
    .map(lineMapping => lineMapping.map(vlq => Base64VLQUtils.encodeMappings(vlq)).join(','))
    .join(';');
  // 写出SourceMap文件
...
async function replaceEnv(envConf, fileList, env) {
  ...
  for (const fileItem of fileList) {
    ...
    // Pro 环境不需要 sourcemap 处理，直接跳过
    let hasSourceMap = FileHelper.isFile(fileItem + '.map') && env !== 'pro';
...
async function deploy({ projectName, env, room, opts, workDir }) {
  ...
  // 搜索并执行替换
  const modifyFileList = await replaceEnv(envConf, dealWidthFileList, env);
  ...
  if (!publicPath) {
    throw ErrorHelper.throwError(ErrorCode.ERROR_DEPLOY_PUBLIC_URL_INVALID);
  }
...
function replaceAllEnv(target, matchKeyPattern, envConf) {
  let newMiddle = target;
  let match = null;
  while ((match = newMiddle.match(matchKeyPattern))) {
    newMiddle = newMiddle.replace(match[0], envConf[match[1]]);
  }
  return newMiddle;
}
