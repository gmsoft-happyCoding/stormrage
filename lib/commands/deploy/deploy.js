...
async function deploy({ projectName, env, room, opts, workDir }) {
  ...
  // 检查target版本是否存在，如果存在则进一步检查是否存在Make版本号，如果没有，则尝试对Make版本进行补全
  if (
    opts.target &&
    opts.target.match(NpmHelper.MAIN_VERSION_PATTERN) &&
    !opts.target.match(NpmHelper.MAKE_VERSION_PATTERN)
  ) {
    const latestVersion = await npmHelper.getLatestVersion(projectName, opts.target);
    if (latestVersion) {
      opts.target = latestVersion;
    }
  }
