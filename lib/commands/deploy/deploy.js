...
async function deploy({ projectName, env, room, opts, workDir }) {
  ...
  const publicPath = validConf.business['public-url'];
...
async function generateConf(allConf, deployType, env, room, publicPath, pwd, distFileName) {
  ...
  if (!deployConf) {
    ...
    deployConf =
      allDeployConf[ConfHelper.CONF_DEFAULT_DEPLOY_FIELD_TO][
        deployType === 'app'
  ...
  const domainPrefix = publicPath.match(/^((https?):)?\/\/[^/]+/);
  if (domainPrefix) {
    safePublicPath = publicPath.replace(domainPrefix, '');
  }
