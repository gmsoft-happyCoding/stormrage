...
async function recalculateHash(allFiles, modifyFiles, depth = 0) {
  const currentAllFile = lodash.cloneDeep(allFiles);
  const currentModifyFiles = Array.from(modifyFiles);

  if (lodash.isEmpty(currentModifyFiles)) {
    return;
  }
  ...
  for (const modifyFilePath of currentModifyFiles) {
    ...
    for (const testFilePath of currentAllFile) {
      ...
      if (fileContent.includes(effectFileName)) {
        const replaceRegex = new RegExp(RegExpUtils.escape(effectFileName), 'g');
        fileContent = fileContent.replace(replaceRegex, path.basename(newHashFilePath));
      ...
      // 匹配样例：2:"b0d52471"
      // 匹配模式：/(?<=\d:")[\da-f]{6,}(?=")/i
      const hashPattern = /(?<=\d\.)([\da-f]{6,})(?=\.chunk.(js|css))/i;
      ...
      // 匹配样例1：2:"b0d52471"
      // 匹配样例2：2:\"b0d52471\"
      // 匹配样例4：_a5c6f111.js or _a5c6f111.css
      const webpackHashPattern1 = oldFileHash
        ? new RegExp(`(?<=(,|\\{)\\d+:\\\\")${oldFileHash[0]}(?=\\\\")`, 'ig')
        : /____/;
      const webpackHashPattern2 = oldFileHash
        ? new RegExp(`(?<=(,|\\{)\\d+:")${oldFileHash[0]}(?=")`, 'ig')
        : /____/;
      const webpackHashPattern3 = oldFileHash
        ? new RegExp(`(?<=_)${oldFileHash[0]}(?=\.(js|css))`, 'ig')
        : /____/;
      // 如果目标文件存在Webpack模块加载系统的Hash，则更新，并记录此次文件更新，用于下一轮迭代
      if (
        oldFileHash &&
        newFileHash &&
        (webpackHashPattern1.test(fileContent) ||
          webpackHashPattern2.test(fileContent) ||
          webpackHashPattern3.test(fileContent)) &&
        fileContent.includes(oldFileHash[0])
      ) {
        // 替换文件Hash
        fileContent = fileContent.replace(webpackHashPattern1, newFileHash[0]);
        fileContent = fileContent.replace(webpackHashPattern2, newFileHash[0]);
        fileContent = fileContent.replace(webpackHashPattern3, newFileHash[0]);
        await fs.writeFile(testFilePath, fileContent, 'utf8');
        // 如果修改的文件名符合Hash重算规则，且不是sourceMap文件，则加入下一轮Hash重算任务
        if (FileHelper.isHashFileNamePattern.test(testFilePath) && !testFilePath.endsWith('.map')) {
          nextModifyFiles.add(testFilePath);
