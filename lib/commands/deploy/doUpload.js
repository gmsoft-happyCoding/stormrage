...
async function doUpload({ targetConf, env, room, deployTag, buildDir, publicUrl }) {
  // ================================================ 发布配置构建 ================================================
  ...
  if (!deployConf) {
    ...
    deployConf = allDeployConf[keyName][deployTag];
  }

  if (!deployConf) {
    ErrorHelper.throwError(ErrorCode.ERROR_DEPLOY_TAG_INVALID);
  }
  ...
  const deployMachines = deployConf.map(conf => {
    ...
    if (targetMachine) {
      const subDir = url.parse(ConfHelper.addProtocol(publicUrl)).pathname;
