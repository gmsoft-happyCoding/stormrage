...
class FileHelper {
  ...
  constructor(filePath) {
    ...
    this.hashBaseNameHistory = [this.baseName];
  }
  ...
  set fileContent(content) {
    ...
    if (FileHelper.isHashFileNamePattern.test(this.baseName)) {
      // 下一轮的Hash文件名
      const nextHashName = this.baseName.replace(
        FileHelper.isHashFileNamePattern,
        this.fileHash.substr(-8)
      );

      // Hash不一样，将当前Hash文件名加入到历史记录中
      if (nextHashName !== this.hashBaseName) {
        this.hashBaseNameHistory.push(nextHashName);
      }

      this.hashBaseName = nextHashName;
    }
