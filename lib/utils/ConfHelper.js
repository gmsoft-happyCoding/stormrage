...
class ConfHelper {
  ...
  static getConfFileName(rootDir, fileName) {
    ...
    if (!confFileName) {
      ...
      const confName = projectInfo.name.toLowerCase().replace(/-/g, '');
      // TODO 此处的命名规则需要明确，暂定
  ...
  static replaceEnvForSelf(envObj, _depth = 0) {
    const env = {};
    ...
    // 检查是否存在未替换的占位符
    let placeholderKey = [];
    lodash.forIn(env, val => {
      if (typeof val !== 'string') {
        return;
      }

      const placeholder = val.match(placeholderPattern);
      const keyIntersection = lodash.intersection(allKey, placeholder);
      if (placeholder && keyIntersection.length > 0) {
        placeholderKey.push(...keyIntersection);
      }
    });

    // 如果出现未替换的占位符，则再次进行替换，最大递归深度：10
    if (placeholderKey.length > 0) {
      if (_depth >= 10) {
        ErrorHelper.throwError(
          ErrorCode.ERROR_CONFIG_RESOLVE_FAILED,
          `[ 相关配置段：${placeholderKey.join(',')} ]`
        );
      }   

      return this.replaceEnvForSelf(env, _depth + 1);
    }
