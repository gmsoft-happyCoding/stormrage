...
class ConfHelper {
  ...
  static async getFullEnvConf(opt) {
    const { rootDir, rooms, confName, envName = 'test1', envField } = opt;
    let envConf = {};
    let allConf = [];
    ...
    for (const room of rooms) {
      //
      const confUrl = this.getConfigUrl(rootDir, envName, room, confName);
      try {
    ...
    const envWhiteList = ['business', 'hosts', 'gateway', 'pdf-preview', ...extraFields];
