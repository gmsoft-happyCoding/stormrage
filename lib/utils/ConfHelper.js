...
class ConfHelper {
  ...
  static convertConfToEnvironment(conf, _parentKey) {
    ...
    keys.forEach(key => {
      ...
      if (typeof conf[key] === 'object' && conf[key].constructor !== Array) {
  ...
  /**
   * 获取环境变量白名单
   * @param {*} extraFields 额外的环境变量字段 逗号分隔的字符串 或者 字符串数组
   */
  static getEnvWhitelist(extraField) {
    let extraFields = [];
    if (typeof extraField === 'string') {
      extraFields = extraField.split(',');
    }
    if (lodash.isArray(extraField)) {
      extraFields = extraField;
    }
    // 默认注入下面的配置段，整个环境变量段并不是所有的段都对前端有意义，暂定这些段，如果有需要可以使用参数进行额外配置
    return [
      this.CONF_DEFAULT_FIELD_NAME,
      this.CONF_DEFAULT_COMPILE_NAME,
      this.CONF_DEFAULT_HOSTS_NAME,
      this.CONF_DEFAULT_GATEWAY_NAME,
      this.CONF_DEFAULT_PDF_PREVIEW_NAME,
      ...extraFields,
    ];
  }
  ...
  static async getProjectDefaultConf(opts, usePlugin = false) {
    ...
    const confFilePath = path.resolve(
      rootDir,
      isFisEnv ? 'project-config/media' : 'project-config',
      confName || 'default.yml'
    );

    const defaultConfOriginal = await this.getYmlConfig(confFilePath, 5000);

    const envWhiteList = this.getEnvWhitelist(envField);

    const confPart = lodash.pick(defaultConfOriginal, envWhiteList);
    // 将环境变量降维展开,转变为一维结构
