...
class ConfHelper {
   ...
   * @param {'app' | 'components'} projectType 构建的项目类型，不同的项目类型默认对应的配置文件也不同
   */
  static getConfigUrl(rootDir, env, room, fileName, confLabel = 1, projectType) {
    let confFileName = fileName;

    if (!confFileName) {
      // 尝试根据当前环境去获取配置文件,规则：项目路径名去除短横线，转小写
      const confName = path.parse(rootDir).name.toLowerCase().replaceAll('-', '');
      // TODO 此处的命名规则需要明确，暂定
      confFileName = projectType
        ? `${this.CONF_PREFIX}${projectType}${confName}`
        : `${this.CONF_PREFIX}${confName}`;
    }
   ...
   * @param {'app' | 'components'} opt.projectType 构建的项目类型，不同的项目类型默认对应的配置文件也不同
   */
  static async getFullEnvConf(opt) {
    const { rootDir, rooms, confName, envName = 'test1', envField, confLabel, projectType } = opt;
    let envConf = {};
    let allConf = [];
    ...
    // 串行配置获取
    for (const room of rooms) {
      const confUrl = this.getConfigUrl(rootDir, envName, room, confName, confLabel, projectType);
      try {
