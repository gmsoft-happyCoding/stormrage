...
const ErrorCode = {
  ...
  /** Make失败：加载主题生成文件失败，无法取得主题配置信息 */
  ERROR_MAKE_INIT_THEME_FAILED: 3004,
  /** Make失败：缺少必要的环境变量定义（business.project-name、business.public-url） */
  ERROR_MAKE_ENV_NOT_FOUND: 3005,
  ...
  /** 配置文件解析失败，配置文件占位符递归深度超限，可能存在循环引用 */
  ERROR_CONFIG_RESOLVE_FAILED: 5003,
  ...
  /** Deploy失败，缺少必要的环境变量定义（business.public-url） */
  ERROR_DEPLOY_PUBLIC_URL_INVALID: 6003,
...
const ErrorMessage = {
  ...
  [ErrorCode.ERROR_PRO_CONFIG_NOT_EXIST]:
    '发布配置文件不存在，请检查传入的发布环境、机房信息是否争取以及对应环境下配置文件是否存在',
  [ErrorCode.ERROR_CONFIG_RESOLVE_FAILED]:
    '配置文件解析失败，占位符处理时递归深度超限，可能存在占位符循环引用，请检查配置文件的占位符间引用关系是否正确',
  [ErrorCode.ERROR_MAKE_INIT_THEME_FAILED]:
    '加载主题生成文件失败，无法取得主题配置信息，请检查主题配置文件正确性',
  [ErrorCode.ERROR_MAKE_ENV_NOT_FOUND]:
    'Make失败，缺少必要的环境变量定义（business.project-name、business.public-url）',
  ...
  [ErrorCode.ERROR_DEPLOY_NAME_OR_VERSION_INVALID]:
    'Deploy失败，拉取成品库失败，请检查发布的项目名称与版本号是否正确',
  [ErrorCode.ERROR_DEPLOY_TAG_INVALID]:
    'Deploy失败，当前项目配置指定了deploy-tag部署参数，但是在部署配置文件中，未发现对应配置项目，请检查deploy-tag参数是否正确，或机房配置是否正确添加',
  [ErrorCode.ERROR_DEPLOY_PUBLIC_URL_INVALID]:
    'Deploy失败，缺少必要的环境变量定义（business.public-url）',
  ...
  [ErrorCode.ERROR_DEV_CONFIG_NOT_EXIST]:
    '启动开发调试所需的配置文件不存在，请在项目的project-conf/project-config目录下新建default.yml配置文件，或使用自定义的文件名，并在启动时进行指定',
...
const TipsMessage = {
  ...
  make: '当前命令用法：stormrage make [localDir] -p <projectType> -e <env> -c <conf> -f <fields> -d <localDest> --svn <url> --svn-checkout-dir <path> --no-force-svn-checkout',
  deploy:
  ...
  start:
    '当前命令用法：stormrage start [localDir] -p <projectType> --port <port> --conf <conFileName> --debugConf <conFileName> --output <path> -f <fields> --next',
};
