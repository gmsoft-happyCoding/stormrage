...
class SvnHelper {
  ...
  static async getList(targetUrl) {
    const dirExists = await this.dirIsExists(targetUrl);
    if (!dirExists) {
      ErrorHelper.throwError(ErrorCode.ERROR_DIR_INVALID);
    }
   ...
   */
  static async dirIsExists(targetUrl) {
    try {
      ...
      if (error.message.includes("Could not list all targets because some targets don't exist")) {
        return false;
      }
  ...
  static async getBaseBranches(rootDir, baseBranchesName) {
    ...
    // 在Tags下寻找
    const inTags = await this.dirIsExists(tagsPath);
    if (inTags) {
      return tagsPath;
    }
    // 在branches下寻找
    const inBranches = await this.dirIsExists(branchesPath);
    if (inBranches) {
      return branchesPath;
    }
    // 在当前项目根路径下寻找
    const inRoot = await this.dirIsExists(rootPath);
    if (inRoot) {
      return rootPath;
    }
    ErrorHelper.throwError(ErrorCode.ERROR_BASE_BRANCHES_NOT_EXIST);
  ...
  static async getNewBranches(rootDir, branchName) {
    ...
    const alreadyExists = await this.dirIsExists(newBranchesPath);
    if (alreadyExists) {
