...
/** Mono项目的特征文件夹名称 */
const monoFeatureDirName = 'packages';
/** 快照版本存放位置 */
...
class SvnHelper {
  ...
  static async updateForkProjectVersion(needUpdateSubVersionPath, branchName, fillVersionName) {
    for (let i = 0; i < needUpdateSubVersionPath.length; i++) {
      ...
      const originalVersionMatch = (await fs.readJSON(packageJsonPath)).version.match(
        /^(\d+\.){2}\d+/
      );
      ...
      const originalVersion = originalVersionMatch[0];
      ...
      await FileHelper.writeFileContent(
        packageJsonPath,
        fileContent.replace(versionPattern, newVersion)
      );
