...
class SvnHelper {
  ...
  static async updateNewBranchesVersion(newBranchesPath) {
    ...
    if (versionsMode === this.VERSION_MODE_MASTER) {
      // 检查当前项目的类型，如果是Mono项目，则需要进行子项目版本号同步更新
      const branchesFiles = await this.getList(newBranchesPath);
      // 找到当前分支下的文件夹
      const branchesDirs = branchesFiles.filter(i => i['$'].kind === 'dir');
      // 如果文件夹数量不为1，则表明该分支不是项目根路径，无法确定项目根路径，抛出异常
      if (branchesDirs.length !== 1) {
        ErrorHelper.throwError(ErrorCode.ERROR_BRANCHES_DIR_INVALID);
      }

      const projectRootDir = `${newBranchesPath}/${branchesDirs[0].name}`;
      ...
      if (packagesDirIsExist) {
        ...
        // 更新新Fork项目的版本号
        await this.updateForkProjectVersion(needUpdateSubVersionPath, branchName);
      }
    }
  }

  static async updateForkProjectVersion(needUpdateSubVersionPath, branchName) {
    for (let i = 0; i < needUpdateSubVersionPath.length; i++) {
      ...
      const newVersion = `${originalVersion}-${branchName}`;
