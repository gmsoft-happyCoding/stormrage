...
class ReleaseHelper extends SvnHelper {
  ...
  static async getSnapshotVersion(rootDir, options) {
    ...
    if (versionMode === this.VERSION_MODE_MASTER) {
      // 取得当前项目的主分支路径
      const projectRootDir = await this.getUniqSubDirectory(`${rootDir}/${mainBranchesName}`);
      // 读取项目的package.json文件，获取当前版本号
      let releaseVersion = await SvnHelper.getCurrentVersionFromPackageJSON(projectRootDir);
