...
const setVSComponent = meta => {
  ...
  if (meta && meta.description && meta.description.includes(flag)) {
    try {
      const vs = meta.description.match(regx)[1];
      return {
        ...meta,
      ...
      console.log('提取 @vs 数据失败, 请检查配置是否正确', e);
...
const parseFile = file => {
  ...
  const meta = compose(flagWorkflowProps, setVSComponent, flagWorkflowComponent)(metaRaw);
  return meta;
};
