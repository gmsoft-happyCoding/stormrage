  ...
  return new Promise((resolve, reject) => {
    ...
    if (verbose)
      sshClient.on("write", function(p) {
        console.log(
          chalk.yellow(p.source + " => " + host + "@" + p.destination)
        );
      });
    ...
    sshClient.on("end", function() {
      endTime = new Date();
      if (connectSuccess)
        console.log(chalk.cyan(host + " [上传结束] " + new Date()));
    });
    ...
    sshClient.on("close", function() {
      if (connectSuccess) {
        console.log(
          chalk.cyan(
            host + " 上传用时: [" + (+endTime - +startTime) / 1000 + "] 秒!"
          )
        );
        resolve();
      }
...
async function scp(
  ...
  if (privateKey === undefined) {
    questions.push({
      type: "input",
      name: "privateKey",
      default: nodeJsPath.resolve(os.userInfo().homedir, ".ssh", "id_rsa"),
      message: "登录私钥路径?",
      when: answers => {
