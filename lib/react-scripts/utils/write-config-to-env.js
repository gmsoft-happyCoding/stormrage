...
const { forEach, isString, uniq } = require('lodash');
...
const writeConfigToEnv = async (pluginOption, env, room) => {
  ...
  const { next, conf = 'default', debugConf = 'debug', field } = program.opts();
  ...
  if (next) {
    ...
    if (isStart) {
      // 读取debug.yml配置文件
      const debugLocalConfPath = path.resolve(
        paths.projectConfig,
        `${debugConf.replace(/\.yml$/, '')}.yml`
      );
    ...
    // 获取过滤白名单
    const envWhiteList = uniq([...Object.keys(confEnv), ...Object.keys(debugConfEnv)]);
