...
const writeConfigToEnv = async (pluginOption, env, room) => {
  // 判断当前是否是start命令
  const isStart = process.argv[1].endsWith('-start.js');

  // 如果是start，则默认读取本地的配置文件（project-config/debug.yml）
  const { next, conf = isStart ? 'debug' : 'default', field } = program.opts();

  // 识别到启用新的环境变量加载模式
  if (next) {
    // 优先读取本地的配置文件（project-config/default.yml）， 读取test1的配置文件
    ...
    // 将配置结构扁平化，方便注入
    const flattedConf = ConfHelper.convertConfToEnvironment(confEnv);
    // 获取过滤白名单
