...
const getClientEnvironment = require(configResolve('env'));
const path = require('path');
const chalk = require('chalk');
const fs = require('fs-extra');
const webpack = require('webpack');
const bfj = require('bfj');
const config = require(configResolve('webpack.config.prod'));
...
module.exports = checkBrowsers(paths.appPath, isInteractive)
  .then(() => {
...
function build(previousFileSizes) {
  ...
  return new Promise((resolve, reject) => {
    compiler.run((err, stats) => {
      ...
      // 禁用CI环境下警告转报错的行为
      const disableCIWarningToError = true;
      if (
        !disableCIWarningToError &&
        process.env.CI &&
...
function compileRefRouter() {
  const refRouterPath = path.join(paths.appBuild, 'ref-router');
  if (!fs.pathExistsSync(refRouterPath)) return;

  const env = getClientEnvironment(paths.servedPath);

  walker.files.sync(refRouterPath).forEach(function (stats) {
    // 修改文件权限为可读写
