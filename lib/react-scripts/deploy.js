...
// 获取项目配置的要发布的机器信息
function getZipFileNameSubjoin(projectConfig) {
  return projectConfig.REACT_APP_DEPLOY_ZIP_FILE_NAME_SUBJOIN;
}
...
async function deploy({ env, room, projectConfig, destDir }) {
  ...
  const zipFileNameSubjoin = getZipFileNameSubjoin(projectConfig);
