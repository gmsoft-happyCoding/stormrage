...
/**
 * 应用配置参数到fis media
 * @param mediaName {string} media 名称
 * @param vars {object} 项目配置参数
 * @param themeVars {object} less 变量
 * @param deploy {object} 部署相关参数
 * @param packConfig {object} 打包配置
 */
module.exports = function useConf(context) {
  ...
  // 是否打包
  if (config.pack && config.packConfig) {
    media.match('::packager', {
      packager: fis.plugin('deps-pack', config.packConfig),
    });
  }
