...
async function run(output, clean, pluginOption) {
  const { next, conf = 'default', debugConf = 'debug' } = program.opts();
  ...
  if (next) {
    ...
    const confEnv = await loadConf(conf);
    const debugConfEnv = await loadConf(debugConf);
    // 获取过滤白名单
    const envWhiteList = ConfHelper.getEnvWhitelist(confEnv);
    // 将配置结构扁平化，方便注入
    let fullEnv = {
    ...
    fullEnv = ConfHelper.replaceEnvForSelf(fullEnv);
